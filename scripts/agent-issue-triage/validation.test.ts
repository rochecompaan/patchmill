...
const issues: IssueSummary[] = [
  { number: 1, title: "Easy bug", body: "Steps and expected behavior", labels: ["bug"], state: "open" },
  { number: 2, title: "Ambiguous feature", body: "Make reports better", labels: ["enhancement"], state: "open" },
];
...
test("validateTriageDocument accepts recommended answers for needs-info decision questions", () => {
  const decisions = validateTriageDocument(doc({
    ...
    questions: [{
      ...
      recommendedAnswer: "Start with CSV export matching the visible dashboard columns.",
    }],
    comment: null,
  }), [issues[1]]);
...
test("validateTriageDocument rejects removed triage labels", () => {
  for (const removedLabel of ["agent-easy", "agent-mechanical", "agent-needs-human-decision", "agent-needs-info", "agent-needs-plan", "needs-plan"]) {
    assert.throws(
      () => validateTriageDocument(doc({
        issueNumber: 1,
        primaryBucket: "agent-ready",
...
test("validateTriageDocument rejects removed primary buckets", () => {
  assert.throws(
    () => validateTriageDocument(doc({
      ...
      labels: ["enhancement", "agent-needs-human-decision", "priority:medium"],
      confidence: "high",
      rationale: "Feature behavior is ambiguous and needs product direction.",
      questions: [],
      comment: null,
...
test("validateTriageDocument accepts plain string needs-info questions", () => {
  ...
  assert.deepEqual(decisions[0].questions, ["What report behavior should be implemented?"]);
});
...
test("validateTriageDocument rejects needs-info question objects without recommended answers", () => {
  assert.throws(
    () => validateTriageDocument(doc({
      ...
      confidence: "high",
      rationale: "Feature behavior is ambiguous and needs product direction.",
      questions: [{ question: "What report behavior should be implemented?", recommendedAnswer: "" }],
      comment: null,
    }), [issues[1]]),
