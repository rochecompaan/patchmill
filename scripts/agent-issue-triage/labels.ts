...
export const DEFAULT_TRIAGE_POLICY = createTriagePolicy(DEFAULT_PATCHMILL_CONFIG.labels);

export const PRIMARY_BUCKETS: PrimaryBucket[] = primaryBucketStatuses(DEFAULT_TRIAGE_POLICY);

export const REQUIRED_LABELS: LabelDefinition[] = DEFAULT_TRIAGE_POLICY.allowedLabels;
...
export const DEFAULT_TRIAGE_EXCLUDED_LABELS = new Set<string>(DEFAULT_TRIAGE_POLICY.excludedLabels);
...
export function primaryBucketLabelMap(policy: PatchmillTriagePolicy): Map<PrimaryBucket, string> {
  return new Map(policy.primaryBuckets.map((bucket) => [bucket.status, bucket.label]));
}

export function primaryBucketLabels(policy: PatchmillTriagePolicy): Set<string> {
  return new Set(policy.primaryBuckets.map((bucket) => bucket.label));
}

export function triageAllowedLabelNames(policy: PatchmillTriagePolicy): Set<string> {
...
export function missingLabelDefinitions(
  ...
  return triagePolicy.allowedLabels.filter((label) => !existing.has(label.name));
}
