...
function event(partial: Partial<AgentIssueProgressEvent>): AgentIssueProgressEvent {
  return {
...
test("console reporter renders subagent tool calls with only agent details", () => {
  ...
  reporter.event(event({ message: "implement task", step: { type: "step-start", label: "implement task" } }));
...
test("console reporter renders subagent management calls as normal tools", () => {
  ...
  reporter.event(event({
    ...
    stage: "pi-implementation",
    message: "tool",
    observation: { type: "tool-call", toolName: "subagent", arguments: { action: "list" } },
  }));
...
test("console reporter suppresses heartbeat and raw text observations", () => {
  const lines: string[] = [];
  const reporter = new AgentIssueConsoleProgressReporter({ writeLine: (line) => lines.push(line), startedAt: BASE });

  reporter.event(event({ message: "issue #19 · Add filters", step: { type: "run-start", issueNumber: 19, title: "Add filters" } }));
  reporter.event(event({ message: "create plan", step: { type: "step-start", label: "create plan" } }));
  reporter.event(event({ level: "heartbeat", stage: "pi-plan", message: "[issue #19] planning | tok: task=1k total=2k | elapsed 4m" }));
  reporter.event(event({ level: "debug", stage: "pi-plan", message: "text", observation: { type: "text", text: "raw skill body" } }));
  reporter.event(event({ time: "2026-05-22T10:00:04.000Z", message: "create plan", step: { type: "step-complete", label: "create plan" } }));
...
test("console reporter renders mandatory implementation task labels", () => {
  ...
  reporter.event(event({ message: "issue #19 · Filters", step: { type: "run-start", issueNumber: 19, title: "Filters" } }));
  reporter.event(event({ message: "implement task 1/6 date range model", step: { type: "step-start", label: "implement task 1/6 date range model" } }));
  reporter.event(event({ level: "debug", stage: "pi-implementation", message: "usage", observation: { type: "assistant-usage", outputTokens: 2100 } }));
  reporter.event(event({ time: "2026-05-22T10:03:08.000Z", message: "implement task 1/6 date range model", step: { type: "step-complete", label: "implement task 1/6 date range model" } }));
  reporter.event(event({ message: "implement task 2/6 dashboard wiring", step: { type: "step-start", label: "implement task 2/6 dashboard wiring" } }));
...
test("console reporter separates completed steps with a blank line", () => {
  ...
  reporter.event(event({ message: "select issue", step: { type: "step-start", label: "select issue" } }));
  reporter.event(event({ time: "2026-05-22T10:00:01.000Z", message: "select issue", step: { type: "step-complete", label: "select issue" } }));
  reporter.event(event({ message: "use existing plan", step: { type: "step-start", label: "use existing plan" } }));
  reporter.event(event({ time: "2026-05-22T10:00:02.000Z", message: "use existing plan", step: { type: "step-complete", label: "use existing plan" } }));
...
test("console reporter uses completion event accounting fields without synthesizing tool-call dots", () => {
  ...
  reporter.event(event({ message: "claim issue", step: { type: "step-start", label: "claim issue" } }));
  reporter.event(event({ observation: { type: "assistant-usage", outputTokens: 400 }, level: "debug", stage: "pi-plan", message: "usage" }));
  reporter.event(event({
    time: "2026-05-22T10:00:05.000Z",
    message: "claim issue",
    step: { type: "step-complete", label: "claim issue", taskOutputTokens: 1200, totalOutputTokens: 2400, toolCalls: 3, elapsedSeconds: 5 },
  }));
