...
test("selectIssue honors custom priority ordering", () => {
  const selected = selectIssue([
    issue(4, [ready, "priority:p0"]),
    issue(2, [ready, "priority:p1"]),
  ], {
    readyLabel: ready,
...
test("selectIssue rejects an explicit open issue that is in progress", () => {
  assert.throws(
    () => selectIssue([
      issue(7, [ready, critical, inProgress]),
      issue(8, [ready]),
    ], { readyLabel: ready, issueNumber: 7 }),
    new RegExp(`Issue #7 is open but not eligible because it has ${inProgress}`),
  );
});
