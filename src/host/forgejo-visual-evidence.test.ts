...
test("ForgejoVisualEvidenceUploader escapes attachment URLs for Markdown destinations", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-markdown-"));
  await mkdir(join(repoRoot, ".tmp"), { recursive: true });
  await writeMinimalPng(join(repoRoot, ".tmp", "dashboard.png"));
  ...
  const uploader = new ForgejoVisualEvidenceUploader({
    ...
    fetchImpl: async (url, init) => {
      if (String(url).endsWith("/assets")) {
        return Response.json({
          browser_download_url: "https://forgejo.example/attachments/dashboard(after image).png",
        });
      }
  ...
  assert.match(commentBody, /!\[Dashboard after change\]\(https:\/\/forgejo\.example\/attachments\/dashboard%28after%20image%29\.png\)/);
});

test("ForgejoVisualEvidenceUploader rejects non-http attachment URLs returned by Forgejo", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-invalid-url-"));
  await mkdir(join(repoRoot, ".tmp"), { recursive: true });
  await writeMinimalPng(join(repoRoot, ".tmp", "dashboard.png"));
...
test("ForgejoVisualEvidenceUploader rejects screenshots outside the repo root", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-outside-"));
...
test("ForgejoVisualEvidenceUploader rejects symlink escapes outside the repo root", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-symlink-"));
  await mkdir(join(repoRoot, ".tmp"), { recursive: true });
...
test("ForgejoVisualEvidenceUploader rejects non-image screenshot paths such as .env", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-env-"));
  await writeFile(join(repoRoot, ".env"), "PATCHMILL_FORGEJO_TOKEN=secret\n", "utf8");
...
test("ForgejoVisualEvidenceUploader rejects text files renamed with an image extension", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-fake-png-"));
  await mkdir(join(repoRoot, ".tmp"), { recursive: true });
  await writeFile(join(repoRoot, ".tmp", "notes.png"), "not really an image", "utf8");
...
test("ForgejoVisualEvidenceUploader sanitizes model-supplied PR comment fields", async () => {
  const repoRoot = await mkdtemp(join(tmpdir(), "forgejo-visual-evidence-sanitize-"));
  await mkdir(join(repoRoot, ".tmp"), { recursive: true });
  await writeMinimalPng(join(repoRoot, ".tmp", "dashboard.png"));
  ...
  await uploader.uploadPrEvidence({
    ...
    evidence: [{
      ...
      referencePaths: ["docs/reference.png\n## injected heading ![image](https://evil.example/img.png)"],
    }],
  });

  const commentRequest = requests.find((request) => request.url.endsWith("/comments"));
  ...
  assert.doesNotMatch(payload.body, /\n## injected heading/);
  assert.doesNotMatch(payload.body, /!\[image\]\(https:\/\/evil\.example\/img\.png\)/);
  assert.match(payload.body, /- Looks good - injected bullet \\\[click me\\\]\\\(https:\/\/evil\.example\\\) \\`inline code\\`/);
  assert.match(payload.body, /- Reference: docs\/reference\.png \\#\\# injected heading \\!\\\[image\\\]\\\(https:\/\/evil\.example\/img\.png\\\)/);
});
