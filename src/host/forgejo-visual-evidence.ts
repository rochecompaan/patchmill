...
function markdownAlt(evidence: AgentIssueVisualEvidence): string {
  return sanitizeMarkdownInline(
    evidence.caption ?? basename(evidence.screenshotPath),
  );
}
...
function visualEvidenceComment(uploaded: AgentIssueVisualEvidence[]): string {
  ...
  for (const evidence of uploaded) {
    ...
    if (!url)
      throw new Error(
        `Visual evidence comment requires an http(s) attachment URL for ${evidence.screenshotPath}`,
      );
    lines.push(`![${markdownAlt(evidence)}](${url})`);
    const caption = evidence.caption
      ? sanitizeMarkdownInline(evidence.caption)
      : undefined;
    if (caption) lines.push(`- ${caption}`);
    lines.push(`- Uploaded screenshot: ${url}`);
    lines.push(
      `- Source path: ${sanitizeMarkdownInline(evidence.screenshotPath)}`,
    );
  ...
  lines.push(
    "If the visual baseline intentionally changed, the relevant committed reference screenshot has been updated in this PR.",
  );
  return lines.join("\n").trimEnd();
}
...
export function hasForgejoVisualEvidenceConfig(
  env: ForgejoVisualEvidenceEnv = process.env,
): boolean {
  return (
    Boolean(envValue(env, "PATCHMILL_FORGEJO_URL")) &&
...
export class ForgejoVisualEvidenceUploader implements VisualEvidenceUploader {
  ...
  private async forgejoConfig(
    ...
    if (!baseUrl || !token) {
      throw new Error(
        "Visual evidence upload requires PATCHMILL_FORGEJO_URL and PATCHMILL_FORGEJO_TOKEN",
      );
    }
  ...
  private async repoSlug(repoRoot: string): Promise<string> {
    ...
    if (result.code !== 0)
      throw new Error(
        `git remote lookup failed: ${result.stderr || result.stdout}`,
      );
    const repo = repoSlugFromRemoteUrl(result.stdout);
    if (!repo || !repo.includes("/")) {
      throw new Error(
        "Cannot determine Forgejo repository slug; set PATCHMILL_FORGEJO_REPO=owner/repo",
      );
    }
    return repo;
  }
  ...
  private async uploadScreenshot(
    ...
    const response = await fetchImpl(
      apiUrl(config, `/issues/${config.prNumber}/assets`),
      {
        method: "POST",
    ...
    if (!response.ok) {
      throw new Error(
        `Forgejo visual evidence upload failed for ${evidence.screenshotPath}: ${response.status} ${await response.text()}`,
      );
    }

    const parsed = (await response.json()) as Record<string, unknown>;
    const url =
      typeof parsed.browser_download_url === "string"
    ...
    if (!normalizedUrl) {
      throw new Error(
        `Forgejo upload response did not include a valid http(s) attachment URL for ${evidence.screenshotPath}`,
      );
    }
  ...
  private async postEvidenceComment(
    ...
    const response = await fetchImpl(
      apiUrl(config, `/issues/${config.prNumber}/comments`),
      {
        method: "POST",
        headers: new Headers({
    ...
    if (!response.ok) {
      throw new Error(
        `Forgejo visual evidence comment failed: ${response.status} ${await response.text()}`,
      );
    }
  }
}
