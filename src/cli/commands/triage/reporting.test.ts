...
test("createObservedChangeEntries reports labels, comments, state, and bucket", () => {
  const before = [
    issue(
      1,
      ["needs-triage", "bug"],
      [{ authorLogin: "bot", body: "old comment" }],
    ),
  ];
  const after = [
    issue(
      ...
      [
        { authorLogin: "bot", body: "old comment" },
        { authorLogin: "bot", body: "## Agent Brief\nImplement CSV export." },
      ],
      "open",
    ),
  ];
...
test("createObservedChangeEntries extracts blockedBy from Patchmill blocked comments", () => {
  ...
  const after = [
    issue(
      3,
      ["blocked", "bug"],
      [{ authorLogin: "bot", body: blockedComment }],
    ),
  ];

  assert.deepEqual(
    createObservedChangeEntries(before, after, stateMap, ["bot"]),
    [
      {
        issueNumber: 3,
        title: "Issue 3",
