...
export async function inspectBlockedRunRecovery(input: {
  ...
  // An ancestor with no unique commits is an empty/stale checkout, not landed
  // issue work. The typed recovery facade performs its pinned-base refresh.
  else if (merged && (divergence?.ahead ?? 0) > 0) kind = "already-merged";
...
export { assessRunRecovery } from "./recovery-assessment.ts";
export { decideRunRecovery } from "./recovery-policy.ts";
...
export function formatRunRecoveryDecision(
  ...
  const classification = decision.assessment.classification;
  ...
  return `Issue #${decision.assessment.issueNumber} will resume (${classification}).`;
}
